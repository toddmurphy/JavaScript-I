...
console.log(
    `Car 33 is ${inventory[32].car_year} ${inventory[32].car_make} ${inventory[32].car_model}.`
)
...
function allCarYears(arr) {
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].car_year) {
            //"if" establishes a true condtion to meet
...
const mapYears = inventory.map(car => car.car_year)
